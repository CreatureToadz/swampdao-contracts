...
contract FlyzBondStakeDepository is Ownable {
    ...
    address public immutable FLYZ; // intermediate reward token from treasury
    ...
    struct Bond {
        uint256 payout; //flyz at the moment of bond
    ...
    function deposit(
        ...
        IFlyzTreasury(treasury).deposit(_amount, principle, profit);

        if (fee != 0) {
            // fee is transferred to dao
            IERC20(FLYZ).safeTransfer(DAO, fee);
        }
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        IFlyzBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);
        ...
        returns (uint256 pendingPayout_)
    {
        ...
        uint256 payout = IsFlyzERC20(sFLYZ).balanceForGons(
