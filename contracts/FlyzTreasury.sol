...
contract FlyzTreasury is Ownable, IFlyzTreasury {
    ...
    address public sFLYZ;
    uint256 public sFLYZQueue; // Delays change to sFLYZ address
    ...
    /**
        @notice allow approved address to deposit an asset for FLYZ
        @param _amount uint
    ...
    function deposit(
        ...
        // mint FLYZ needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(FLYZ).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn FLYZ for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = valueOfToken(_token, _amount);
        IFlyzERC20(FLYZ).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sFLYZ).balanceOf(msg.sender); // Can only borrow against sFLYZ held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with FLYZ
        @param _amount uint
     */
    function repayDebtWithFLYZ(uint256 _amount) external {
        require(isDebtor[msg.sender], 'Not approved');

        IFlyzERC20(FLYZ).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, FLYZ, _amount, _amount);
    }
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(IERC20(FLYZ).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns FLYZ valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match FLYZ decimals
            value_ = _amount.mul(10**IERC20(FLYZ).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SFLYZ) {
            // 9
            sFLYZQueue = block.number.add(blocksNeededForQueue);
        } else return false;
